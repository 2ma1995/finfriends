...
## 8. 처리 결과 — 받는 쪽 (2026-09-01)

**끝났다.** `feat/parent-mission` 에서 처리했고 어긋남 대장 `D22` 에 기록했다.

| 부탁 | 한 것 |
| --- | --- |
| ① `modules/bank` 가 `getBalance` 를 읽게 | 했다. 🔴 원장 표를 직접 보지 않고 **공개 함수만** 부른다 |
| ② 충전을 `topUp` 으로 | 했다. `topUpAllowance(childId, amount)` → `topUp(…, \`topup:<uuid>\`)` |
| ③ `mock_balance_won` 제거 | 마이그레이션 `20260901000018_bank_on_ledger`. **`savings_interest_pct` · `mock_card_*` 는 그대로 뒀다** |
| ④ 기록 목록 + 「고치기」 | `/parent/bank` 안으로. 못 되돌린 금액을 그대로 말한다(§5) |
| ⑤ `/parent/allowance` 삭제 | 삭제. `/screens` 색인의 죽은 링크 `/parent/missions` 도 같이 지웠다 |

**금액 입력란은 두지 않았다** — 5,000 / 10,000 / 30,000 버튼뿐이다.
입력란을 두면 시연에서 실제 이체처럼 읽힌다. `topUp` 의 임의 금액 경로는 그대로 살아 있으니
아이 쪽에서 필요하면 쓸 수 있다.

**🔴 컬럼에 있던 60,000원은 원장으로 옮기지 않았다.** 원장은 실제로 일어난 일을 적는 곳이고
그 숫자는 보호자가 준 돈이 아니라 시연 버튼이 올린 값이다. 옮기면 아이 화면에
「용돈을 받았어요 60,000원」이라는 없던 기록이 생긴다.

**검증** — `node tools/verify_bank_ledger.mjs` 18건. 부모·아이 잔액 일치 · 0 미만 차단 ·
중복 한 줄 · 되돌림이 원래 줄을 남기는지 · 되돌릴 수 있는 만큼만 · **별을 만들지 않는지**(P-21).
