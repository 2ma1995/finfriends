 ...
 *    여기서 지금 아이에게 다시 붙인다.
 *
 * 사용: node prisma/seed-missions.mjs [아이이름]
 */
...
// 🔴 네 가지 상태를 모두 넣는다 — 「승인 대기」와 「미실천」이 화면에서
//    구별되는지(AC-6.2) 시드만으로 확인할 수 있어야 한다
await prisma.mission.createMany({ data: [
  { childId: c.id, guardianId: c.guardianId, title: "장 볼 때 가격표 두 개 비교하기", topic: "SPEND", reward: 2 },
  { childId: c.id, guardianId: c.guardianId, title: "용돈 기입장 오늘 치 적기",       topic: "SAVE",  reward: 1 },
  { childId: c.id, guardianId: c.guardianId, title: "안 쓰는 물건 하나 정리해 팔기",   topic: "EARN",  reward: 3 },
  { childId: c.id, guardianId: c.guardianId, title: "간식 사기 전에 잠깐 참아 보기",   topic: "SPEND", reward: 2,
    doneAt: day(1), cycleId: cyc },
  { childId: c.id, guardianId: c.guardianId, title: "저금통에 1000원 넣기",          topic: "SAVE",  reward: 1,
    state: "APPROVED", doneAt: day(4), decidedAt: day(3), cycleId: cyc },
  { childId: c.id, guardianId: c.guardianId, title: "심부름 하고 받은 돈 기록하기",   topic: "EARN",  reward: 2,
    state: "BACKFILLED", doneAt: day(9), decidedAt: day(2), cycleId: cyc },
  { childId: c.id, guardianId: c.guardianId, title: "일주일 동안 군것질 안 하기",      topic: "SPEND", reward: 3,
...
console.log("  미션      ", await prisma.mission.count({ where: { childId: c.id } }), "건 (안 함 3 · 대기 1 · 승인 1 · 소급 1 · 거절 1)");
