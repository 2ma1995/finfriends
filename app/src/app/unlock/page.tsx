...
export default async function UnlockPage({
  ...
  return (
    <Screen role="부모 화면" title={title}>
      <Card>
        <p className="text-[0.88em] leading-relaxed">{lead}</p>
      </Card>

      {sp.err ? (
        <div className="mt-2">
          <Card tone="miss">
            <p className="text-[0.88em]">{errors[sp.err] ?? errors.WRONG}</p>
            {/* 🔴 남은 횟수를 말한다. 안 말하면 갑자기 잠긴 것으로 보인다 */}
            {sp.err === "WRONG" && sp.left ? (
              <p className="mt-1 text-[0.84em] text-ink-soft">{triesLeft(Number(sp.left))}</p>
            ) : null}
            {locked ? <p className="mt-1 text-[0.82em] leading-relaxed text-ink-soft">{lockedHelp}</p> : null}
          </Card>
        </div>
      ) : null}
      ...
      {!locked ? (
        <form action={unlockAction} className="mt-3 grid gap-2">
          <label className="grid gap-1">
            <span className="text-[0.78em] text-ink-mute">{pinLabel}</span>
            {/*
          ...
          {/* 🔴 잠깐만 열린다는 것을 미리 말한다 */}
          <p className="text-[0.76em] leading-relaxed text-ink-mute">{graceNotice}</p>
        </form>
      ) : (
        <Link
          href="/login"
