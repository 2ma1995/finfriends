...
import { topUpAction } from "@/app/actions/allowance";
import {
  amountLabel, balanceLabel, errorNotice, historyTitle, memoLabel, memoPlaceholder,
  needLogin, noChild, notice, savedNotice, starSeparation, submitLabel, title,
      ...
      {history.length > 0 ? (
        <>
          ...
          <ul className="grid gap-1">
            {history.map((h) => (
              <li key={h.id} className="flex items-center gap-2 rounded-card border border-line bg-surface px-3 py-2">
