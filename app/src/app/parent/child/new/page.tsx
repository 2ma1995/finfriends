    ...
    >
      <form action={saveChildProfileAction} className="grid gap-2.5">
        <div className="grid gap-1">
          <Label htmlFor="displayName" className="text-[0.8em] font-normal text-ink-soft">
            아이가 화면에서 볼 이름
          </Label>
          <Input
            id="displayName"
          ...
          />
          <small className="text-[0.74em] leading-relaxed text-ink-mute">
            별명도 괜찮아요. 실명 확인에 쓰지 않습니다.
          </small>
        </div>

        <div className="grid gap-1">
          <Label htmlFor="birthYear" className="text-[0.8em] font-normal text-ink-soft">
            태어난 해
          </Label>
          <Input
            id="birthYear"
            ...
            className="min-h-touch rounded-card border-line bg-surface px-3 text-[0.9em] tabular-nums text-ink placeholder:text-ink-mute"
          />
          <small className="text-[0.74em] leading-relaxed text-ink-mute">
        ...
        <fieldset className="grid gap-1">
          <legend className="mb-1 text-[0.8em] text-ink-soft">아이가 쓸 기기</legend>
          <div className="grid gap-1">
            {DEVICE_TYPES.map((d) => (
              <label
                key={d.value}
              ...
              >
                ...
                <span className="flex-1 text-[0.84em] leading-relaxed">
                  {d.label}
                  <br />
                  <small className="text-[0.86em] text-ink-mute">{d.hint}</small>
                </span>
              </label>
            ))}
          </div>
      ...
      <p className="mt-3 text-center text-[0.78em]">
