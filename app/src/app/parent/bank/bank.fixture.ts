...
export const interestNotice = {
  /** 🔴 화면에 「이자」가 두 군데 있다. **어느 돈에 붙는 이자인지**를 제목에서부터 말한다 */
  title: "모으기 목표 이자율",
  current: "목표에 주는 이자율",
  body: "아이가 **모으기 목표(위시리스트)**에 넣어 둔 돈에 부모님이 이자를 줍니다. 「불리기」 나무가 자라는 실천이 됩니다.",
  /**
   * 🔴 「우리 집 적금」의 이자율과 **다른 것**이다. 같은 화면에 둘 다 있어서
   *    이름이 「이자율」 하나면 보호자가 어느 쪽을 정하는지 알 수 없다 — 실제로 못 알아봤다.
   */
  vsSavings: "「우리 집 적금」의 이자는 이것과 별개예요. 그건 아이가 신청할 때마다 아래에서 따로 정합니다.",
