  ...
  return (
    <Screen role="부모 화면" title={title} back={{ href: "/parent/mypage", label: "내 정보" }}>
      {sp.error ? (
        <div className="mb-2">
          <Card tone="miss">
            <p className="text-[0.88em]">{sp.error === "CONFIRM" ? needConfirm : failed}</p>
          </Card>
        </div>
      ) : null}
      ...
      <Card tone="miss">
        <b className="text-[0.88em]">{warn.title}</b>
        <p className="mt-1 text-[0.86em] leading-relaxed text-ink-soft">{warn.body}</p>
      </Card>

      {/* 🔴 무엇을 잃는지 숫자로 보여준다. 「기록이 지워집니다」는 크기를 감춘다 */}
      {p.childName ? (
        <div className="mt-2.5 rounded-card border border-line-2 bg-sand p-3">
          <span className="block text-[0.76em] text-ink-mute">{p.childName}의 기록</span>
          <dl className="mt-1.5 grid gap-1 text-[0.84em]">
      ...
      <div className="mt-2.5">
        <Card>
          <b className="text-[0.82em]">{refundNotice.title}</b>
          <p className="mt-1 text-[0.86em] leading-relaxed text-ink-soft">{refundNotice.body}</p>
          <p className="mt-1.5 text-[0.8em] leading-relaxed text-ink-mute">{starNotice}</p>
        </Card>
      </div>
      ...
      <form action={withdrawAction} className="mt-4 grid gap-2">
        <label className="flex items-start gap-2 rounded-card border border-line bg-surface p-3">
          ...
          <span className="text-[0.84em] leading-relaxed">{confirmLabel}</span>
        </label>
        <button className="min-h-touch w-full rounded-card border border-miss-line bg-miss-bg text-[0.9em] font-bold text-miss">
          {submitLabel}
        </button>
      </form>
      ...
      <Link
        href="/parent/mypage"
        className="mt-2 flex min-h-touch w-full items-center justify-center rounded-card border border-line-2 bg-surface text-[0.86em] text-ink-soft"
      >
        {cancelLabel}
      </Link>
    </Screen>
