...
export async function signOutAction() {
  ...
  /**
   * 🔴 **`/login` 이 아니라 `/` 로 보낸다** (어긋남 대책 D71 · 2026-09-02 사용자 지적).
   *
   *    「로그아웃 상태에서 앱을 껐다가 다시 켜면 로그인 화면이 바로 나오고
   *    랜딩이 안 나온다」 — 홈 화면 앱(standalone)은 다시 열 때
   *    **마지막 화면을 복원**한다. `start_url` 로 새로 가지 않는다.
   *    그래서 로그아웃하고 `/login` 에 서 있었으면 그 화면이 되살아난다.
   *
   *    `/` 로 보내면 쿠키가 없으니 미들웨어가 랜딩으로 rewrite 한다 —
   *    껐다 켜도 복원되는 자리가 `/` 이므로 **랜딩이 나온다.**
   *
   * 🔴 나가는 길이 막히지 않는다. 랜딩 머리에 「로그인」이 있다 (`D67`) —
   *    그게 없을 때 이걸 했으면 다시 들어올 방법이 없었다.
   */
  redirect("/");
}
