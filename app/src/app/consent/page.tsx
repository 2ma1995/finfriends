        ...
        <ul className="mt-2 grid gap-1">
          {CONSENT_ITEMS.filter((i) => i.required).map((i) => (
            <li key={i.key} className="flex items-start gap-2 rounded-card border border-line bg-surface px-3 py-2">
              <span className="mt-0.5 text-body text-primary">☑</span>
              <span className="flex-1 text-sub leading-relaxed text-ink-soft">{i.label}</span>
            </li>
          ))}
        </ul>
      ...
      <form action={completeConsentAction}>
            ...
            <li key={i.key}>
              ...
              >
                ...
                <span className="flex-1 text-sub leading-relaxed">
                  {i.label}
                  {i.required ? <b className="ml-1 text-sub text-miss">필수</b> : null}
                </span>
              </label>
            </li>
          ))}
        ...
        <div className="mt-2">
          <Card>
            ...
            <p className="mt-1 text-sub leading-relaxed text-ink-soft">{NOT_COLLECTED.join(" · ")}</p>
          </Card>
        </div>

        {error ? (
          <p className="mt-2 rounded-card border border-miss-line bg-miss-bg px-3 py-2 text-sub leading-relaxed text-miss">
            {error}
          </p>
        ) : null}
        ...
        <button
          type="submit"
          className="mt-3 min-h-touch w-full rounded-card bg-primary text-body font-bold text-white"
        >
          동의하고 시작하기
        </button>
      </form>

      <p className="mt-2 text-center text-cap text-ink-mute">{GATE_NOTICE}</p>
    </Screen>
  );
}
