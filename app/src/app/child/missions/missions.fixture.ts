...
export const photoResult: Record<string, string> = {
  ...
  NEED: "사진을 안 붙였어요. 한 장 찍어서 보내거나, 「부모님이 직접 보실래요」를 눌러요.",
};
/**
 * 사진 대신 **직접 검사 맞는 길** — 🔴 사용자 지적.
 *
 * 부모가 낸 벌기는 사진이 필수지만, 사진이 증거의 **유일한 형태**는 아니다.
 * 부모가 집에 와서 직접 보고 확인하면 그게 더 확실하다 — 원래 그렇게 하던 일이다.
 *
 * 🔴 **없으면 아이가 갇힌다.** 사진을 못 찍는 사정은 흔하다 — 폰이 없거나,
 *    이미 치웠거나, 찍을 수 없는 일(심부름)이거나. 그때 「했어요」가 아예
 *    안 눌리면 아이는 **한 일을 올릴 방법이 없다.**
 *
 * 🔴 「사진 없이」가 아니라 **「부모님이 직접 보실래요」**라고 적는다.
 *    빠뜨린 것이 아니라 **고른 것**이어야 한다 — 부모도 그렇게 읽는다.
 */
export const checkLaterLabel = "부모님이 직접 보실래요";
export const checkLaterWhy = "사진을 못 찍었으면 이걸 눌러요. 부모님이 오시면 보여 드리고 확인받아요.";
