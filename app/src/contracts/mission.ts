...
export type MissionView = {
  ...
  /**
   * 🔴 **72시간이 지나 자동으로 완료됐다** (어긋남 대장 D51).
   *    부모가 칭찬한 것과 시간이 지나 끝난 것은 아이에게 **다른 말**이다 —
   *    「부모님이 확인했어요」와 「기간이 지나서 완료됐어요」.
   *    별을 받은 것은 같으므로 자리는 같고 문구만 갈린다.
   */
...
/**
 * 사진을 **받나 · 받아야 하나** — 🔴 **셋으로 갈린다.**
 *
 * 아이·부모 화면과 서버가 **같은 답을 내야 한다.** 한쪽만 알면 아이는 사진칸이
 * 없는데 부모는 사진을 기다리거나, 반대가 된다. 그래서 계약에 둔다.
 *
 * 기준은 **앱이 스스로 확인할 수 있느냐**다.
 *
 *   REQUIRED  벌기 · 부모가 준 미션   집안일 · 심부름 — 앱이 볼 수 없다.
 *                                     **부모가 시킨 일이라 증거가 있어야 한다**
 *   OPTIONAL  벌기 · 배워서 한 것     같은 「앱이 볼 수 없는 일」이지만
 *                                     **아이가 스스로 고른 실천**이다.
 *                                     보여주고 싶으면 붙이고, 아니면 부모가
 *                                     나중에 물어서 확인한다 (`FR-032` 「선택」 그대로)
 *   NONE      잘 쓰기 · 모으기 · 불리기  카드 내역 · 통장 · 적금 회차가 답한다.
 *                                     **앱이 아는 것을 다시 찍어 오라고 하지 않는다**
 *
 * 🔴 **`FR-032` 는 사진을 「선택」으로 뒀다** (어긋남 대장 D49).
 *    `REQUIRED` 한 갈래만 그 요건에서 벗어난 것이고, 사용자 결정이다.
 *
 * 🔴 **`REQUIRED` 를 넓히지 않는다.** 찍을 수 없는 실천(참기 · 기록하기)이 실제로 있고,
 *    전부 필수로 만들면 그런 실천은 **아예 올릴 수 없게 된다.**
 */
export type PhotoRule = "REQUIRED" | "OPTIONAL" | "NONE";

export function photoRuleOf(topic: Topic, fromLesson: boolean): PhotoRule {
  if (topic !== "EARN") return "NONE";
  return fromLesson ? "OPTIONAL" : "REQUIRED";
}
