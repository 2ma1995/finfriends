...
> 🔴 **「통과」는 도구가 «본 것»에 대한 답이지, 내가 «궁금한 것»에 대한 답이 아니다.**
>
> 아래 줄들은 전부 같은 실수다 — 2026-09-02~03 에 **여덟 번** 나왔다.
> 주석 개수는 짝을 봤지 본문을 안 봤고, 충돌 표식은 표식을 봤지 내용을 안 봤고,
> `curl` 은 요청을 보냈지 쿠키가 지워졌는지는 안 봤고,
> 게이트는 두 파일이 서로 맞는지는 봤지 그게 최신인지는 안 봤다.
>
> **무엇이 통과했는지 모르겠으면 통과한 게 아니다.**

| 증상 | 원인 |
| --- | --- |
| 고친 화면이 옛것으로 보인다 | `pull` 안 함 · `.next` 가 낡음 |
| `Cannot read properties of undefined` | `db:generate` 안 함 (스키마가 바뀌었다) |
| **안 건드린 파일에서** 「없는 속성」 타입 오류 | 🔴 스키마가 바뀐 커밋을 받았다 — `npm run db:generate`. 코드는 멀쩡한데 **타입만 틀린 것처럼** 보인다 |
...
| **빌드 로그엔 있는데 요청하면 404** | 🔴 **좀비 서버.** 래퍼만 죽고 `next-server` 가 포트를 붙들고 있다 |
| 새로 만든 라우트가 404 · DB 가 딴 것으로 보인다 | 🔴 서버가 **다른 포트로 밀렸다.** 로그의 `Local:` 줄을 읽는다 |
| 「고친 디자인이 배포에 안 보인다」 | 원본과 배포본이 **다른 파일**이다 — `web/` 과 `app/public/` |
| 두 파일이 같은지 볼 때 | 🔴 **제목만 견주면 안 된다.** 문구가 같아도 레이아웃이 다르다 — 줄 수·구역 id·`data-*` 를 센다 |
| 남의 변경이 **충돌 없이** 사라졌다 | 🔴 내 파일이 **옛 상태**였다. `git merge-base HEAD origin/main` 이 상대 커밋이면 그 모양이다 |
| 충돌 표식이 0개다 | 해소된 것이 아닐 수 있다 — **충돌할 것이 없었던** 경우다 |
| 주석 안 내용이 화면에 나온다 | 🔴 주석 **안**에 닫는 표시를 글자로 적었다. 개수는 맞아도 샌다 — **그려서 본문을 본다** |
